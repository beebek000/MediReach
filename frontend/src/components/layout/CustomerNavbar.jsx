...
export default function CustomerNavbar() {
  ...
  const links = [...navLinks, ...(user?.role === ROLES.CUSTOMER ? customerSpecificLinks : [])];
          ...
          <div className="flex items-center justify-between h-16 sm:h-20 gap-4 sm:gap-8">
            ...
            <div className="flex items-center gap-8 lg:gap-12">
              <Link to="/" className="shrink-0 flex items-center">
                <img src={logo} alt="MediReach Logo" className="h-12 sm:h-14 w-auto transition-transform hover:scale-105" />
              </Link>
              
              {/* Desktop Nav Links */}
              <div className="hidden lg:flex items-center gap-2">
                {links.map((link) => {
                  const isActive = location.pathname === link.to || (link.to !== "/" && location.pathname.startsWith(link.to + "/") && link.to !== "/customer");
                  return (
                    <Link
                      key={link.to}
                      to={link.to}
                      className={`px-2 py-2 relative text-sm font-medium transition-colors ${
                        isActive 
                          ? "text-primary after:content-[''] after:absolute after:w-full after:h-[2px] after:bottom-0 after:left-0 after:bg-primary" 
                          : "text-charcoal/70 hover:text-primary hover-underline-animation"
                      }`}
                    >
                      {link.label}
                    </Link>
            ...
            <div className="flex items-center gap-3 sm:gap-5 ml-auto">
              ...
              <div className="hidden sm:block ml-1 sm:ml-4 border-l border-charcoal/10 pl-4 sm:pl-6">
                {user ? (
                  <div className="relative">
                    ...
                    >
                      ...
                      <div className="text-left hidden md:block">
                        <p className="text-sm font-semibold text-charcoal leading-tight">{user.name}</p>
                        <p className="text-[11px] font-medium text-primary mt-0.5 leading-tight uppercase tracking-wide">{user.role}</p>
                      </div>
                      <svg className={`w-4 h-4 text-charcoal/50 transition-transform hidden md:block ${profileDropdownOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" />
                      </svg>
                    </button>
                    {profileDropdownOpen && (
                      <>
                        <div className="fixed inset-0 z-10" onClick={() => setProfileDropdownOpen(false)} aria-hidden />
                  ...
                  <div className="flex items-center gap-3">
                    <Link to="/login" className="px-4 py-2 text-sm font-medium text-charcoal/80 hover:text-primary hover:bg-white/50 rounded-xl transition-colors">
                      Sign In
                    </Link>
                    <Link to="/register" className="rounded-xl bg-primary px-5 py-2.5 text-sm font-medium text-white shadow-lg shadow-primary/30 hover:shadow-primary/50 hover:-translate-y-0.5 transition-all duration-300">
                      Get Started
                    </Link>
                  </div>
                )}
              ...
              >
                <div className="w-5 h-4 flex flex-col justify-between">
                  <span className={`block h-[2px] w-full bg-current transition-transform ${mobileMenuOpen ? 'rotate-45 translate-y-[7px]' : ''}`}></span>
                  <span className={`block h-[2px] w-full bg-current transition-opacity ${mobileMenuOpen ? 'opacity-0' : ''}`}></span>
                  <span className={`block h-[2px] w-full bg-current transition-transform ${mobileMenuOpen ? '-rotate-45 -translate-y-[7px]' : ''}`}></span>
                </div>
              </button>
            </div>
                ...
                return (
                  <Link
                    ...
                    className={`block px-4 py-3 rounded-xl text-base font-medium transition-colors ${
                      isActive ? "bg-primary/10 text-primary" : "text-charcoal/80 hover:bg-charcoal/5"
                    }`}
                  >
                    {link.label}
                  </Link>
